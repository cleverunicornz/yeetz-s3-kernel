...
pub async fn run_real_s3_aba_probe(config: &S3Config) -> Result<Vec<String>, String> {
    ...
    // delete exactly the objects we created, then assert both run-scoped
    // object prefixes are gone. Completed/already-aborted upload ids reject
    // abort; those errors are expected and the list witness below decides
    // whether any in-progress upload remains.
