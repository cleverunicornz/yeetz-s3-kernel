...
`.github/workflows/ci-dev.yml` executes `cargo nextest run --workspace` on
...
tests under `cargo nextest run --workspace` in the `gates` task of the
current `.github/workflows/ci-dev.yml`. Coverage names the deciding
executable only; outcomes are recorded by witnesses, not here.

| Leg | Decision | Coverage |
|---|---|---|
| P1 | r1 serves verified envelopes incl. seq 0 under a floor | `cargo nextest run --workspace` |
| P2 | r2 request shape is exact (read_event) | `cargo nextest run --workspace` |
| P3 | r3/r10 boundaries and stored-integrity corruption are typed and distinct | `cargo nextest run --workspace` |
| P4 | r4 argument preflight is effect-free | `cargo nextest run --workspace` |
| P5 | r5/r10 window served complete or typed error | `cargo nextest run --workspace` |
| P6 | r6 reached_end is a window boundary, not EOF | `cargo nextest run --workspace` |
| P7 | r7 resume walks exactly; u64::MAX without overflow | `cargo nextest run --workspace` |
| P8 | r8 request shape: no log LIST, tail, or writes | `cargo nextest run --workspace` |
| P9 | r9 envelope surface, digests, EventRef agree | `cargo nextest run --workspace`; field privacy manual (source inspection of `crates/yeetz-s3-streams/src/envelope.rs`) |
| F1 | r4/r3 detect a premature effect | `cargo nextest run --workspace` |
| F2 | r2/r8 detect an out-of-shape request | `cargo nextest run --workspace` |
| F3 | r3/r10 detect a mistyped outcome | `cargo nextest run --workspace` |
| F4 | r5 detects missing-history success | `cargo nextest run --workspace` |
| F5 | r5/r6 detect a wrong boundary claim | `cargo nextest run --workspace` |
| F6 | r7 detects gap/duplicate; MAX detects overflow | `cargo nextest run --workspace` |
| F7 | r9 detects digest/ref interchange or loss | `cargo nextest run --workspace` |
